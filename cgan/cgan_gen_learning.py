...
parser.add_argument("--gen_pretrain",type=int,default=0,help="Action classifier pretrained or not")
...
if opt.mode == 'train':
    ...
    ckpt_dir = os.path.join('/fast-stripe/workspaces/deval/synthetic-data/cgan_gen_learning',opt.expt,'checkpoints')
    images_dir = os.path.join('/fast-stripe/workspaces/deval/synthetic-data/cgan_gen_learning',opt.expt,'images')
    videos_dir = os.path.join('/fast-stripe/workspaces/deval/synthetic-data/cgan_gen_learning',opt.expt,'videos')
    models_dir = os.path.join('/fast-stripe/workspaces/deval/synthetic-data/cgan_gen_learning',opt.expt,'models')
    ...
    if opt.temporal_pattern == 'interpolate':
        ...
        test_data = NTURGBDData1(opt.temporal_length,'interpolate',0,'hcn','sub',opt.normalize,opt.centering,opt.spherical,0,0,'test',opt.expt)
    else:
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (imgs, labels) in enumerate(dataloader):
            ...
            # --------------------------------
            #  Train Action Classifier within
            # --------------------------------
            ...
            action_loss = action_loss_real+ opt.lambda_gen * action_loss_gen
            #action_loss = action_loss_gen
            #action_loss.backward()
            
            ## Update which networks
            #optimizer_hcn.step()
            optimizer_G.step()
            #optimizer_hcn.step()
            #optimizer_hcn_dup.step()
            ...
            print(
                "[Epoch %d/%d] [Batch %d/%d] [Act_loss: %f] [Train_gen_acc: %f] [Train_real_acc: %f] [LR_GEN: %f] [LR_ACT: %f] [Test_acc: %f] [Best Test acc: %f]"
                % (epoch, opt.n_epochs, i, len(dataloader), action_loss.item(), train_acc_gen, train_acc_real,optimizer_G.param_groups[0]['lr'], optimizer_hcn.param_groups[0]['lr'],\
        ...
        writer.add_scalar('Total/Loss',generator_loss.avg,epoch)
