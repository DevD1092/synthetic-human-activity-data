...
##Commands:
#Train : python cgan_learning_new3.py --img_width 60 --img_height 25 --channels 3 --temporal_length 60 --temporal_pattern interpolate --num_per 1 --centering 1 --dataset hcn --sample_interval 4000 --save_steps 100 --n_epochs 400 --batch_size 64 --latent_dim 512 --conf_thresh 0.55 --lambda_gen 0.5 --real_per 25 --gen_pretrain 1 --disc_pretrain 1 --act_pretrain 1 --multi_act 0 --expt realper25_z512_actpretrain_genanddiscpretrain_lambda0p5_combinedloss_discconf0p5_reallabels_per1_ep400
...
if opt.mode == 'train':
    ...
    ckpt_dir = os.path.join('/fast-stripe/workspaces/deval/synthetic-data/cgan_gen_learning_new/gen_first',opt.arch,opt.expt,'checkpoints')
    images_dir = os.path.join('/fast-stripe/workspaces/deval/synthetic-data/cgan_gen_learning_new/gen_first',opt.arch,opt.expt,'images')
    videos_dir = os.path.join('/fast-stripe/workspaces/deval/synthetic-data/cgan_gen_learning_new/gen_first',opt.arch,opt.expt,'videos')
    models_dir = os.path.join('/fast-stripe/workspaces/deval/synthetic-data/cgan_gen_learning_new/gen_first',opt.arch,opt.expt,'models')
    ...
    #Loss functions
    if opt.arch == 'mlp' or opt.arch == 'cnn':
    ...
    if opt.arch == 'mlp':
        ...
        discriminator = Discriminator_cnn(d=256, classes=opt.n_classes)
    ...
    if opt.act_pretrain == 1:
        ...
        elif opt.real_per == 10:
            hcn_model_path = '/fast-stripe/workspaces/deval/synthetic-data/hcn/clall_len60_per1_realper10/model_state.pth'
    ...
    if opt.gen_pretrain == 1:
        ...
        elif opt.real_per == 25:
            gen_model_path = '/fast-stripe/workspaces/deval/synthetic-data/cgan/realper25_z512_center_mlp_per1_ep5k/models/5000/model_state_25.pth'
        elif opt.real_per == 10:
            if opt.arch == 'mlp':
                gen_model_path = '/fast-stripe/workspaces/deval/synthetic-data/cgan/realper10_z512_center_mlp_mseloss_per1_ep5k/models/5000/model_state_10.pth'
            elif opt.arch == 'cnn':
                gen_model_path = '/fast-stripe/workspaces/deval/synthetic-data/cgan/realper10_z512_hcndata_cnn_sigmoid_mseloss_linearblock_per1_ep500/models/500/model_state_10.pth'
    ...
    if opt.disc_pretrain == 1:
        if opt.real_per == 100:
            ...
            disc_model_path = '/fast-stripe/workspaces/deval/synthetic-data/cgan/realper25_z512_center_mlp_per1_ep5k/models/5000/model_state_25.pth'
        elif opt.real_per == 10:
            if opt.arch == 'mlp':
                disc_model_path = '/fast-stripe/workspaces/deval/synthetic-data/cgan/realper10_z512_center_mlp_mseloss_per1_ep5k/models/5000/model_state_10.pth'
            elif opt.arch == 'cnn':
                disc_model_path = '/fast-stripe/workspaces/deval/synthetic-data/cgan/realper10_z512_hcndata_cnn_sigmoid_mseloss_linearblock_per1_ep500/models/500/model_state_10.pth'
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (imgs, labels) in enumerate(dataloader):
            ...
            if opt.arch == 'mlp':
                ...
                probs_validity = F.sigmoid(validity_fake)
                d_fake_loss = adversarial_loss(validity_fake, fake)
            elif opt.arch == 'cnn':
                gen_imgs_detach = gen_imgs.detach()
                d_fake_loss = adversarial_loss(discriminator(gen_imgs_detach,gen_y_for_D).squeeze(), fake)
                probs_validity = discriminator(gen_imgs_detach,gen_y_for_D).squeeze()
                gen_imgs_detach = gen_imgs_detach.permute(0,1,3,2)
            ...
            if opt.num_per == 1:
                ...
                #print ("Gen images shape",gen_imgs_detach.shape)
            ...
            if opt.arch == 'cnn':
                probs_validity = np.expand_dims(probs_validity,axis=1)
            #print ("Probs validity",probs_validity.shape)
            ...
            if sum(toKeep) != 0:
                #print ("Lambda",lambda_gen)
                #print ("To Keep",sum(toKeep))
                fakeClassifierLoss = criterion(outputs_gen[toKeep], predictedLabels[toKeep]) * lambda_gen
                #print ("Gen loss",fakeClassifierLoss)
                #fakeClassifierLoss = criterion(outputs_gen[toKeep], labels[toKeep])
                fakeClassifierLoss.backward()
                optimizer_hcn.step()
                train_acc_gen = calc_accuracy(outputs_gen[toKeep],predictedLabels[toKeep])
            ...
            if sum(toKeep) != 0:
                print(
                    ...
                    % (epoch, opt.n_epochs, i, len(dataloader), action_loss_real.item(), fakeClassifierLoss.item(), train_acc_gen, train_acc_real,optimizer_G.param_groups[0]['lr'], optimizer_hcn.param_groups[0]['lr'],\
                        last_ep_test_acc, best_test_acc, sum(toKeep))
                )
            else:
                print(
                    "[Epoch %d/%d] [Batch %d/%d] [loss_real: %f] [loss_gen: %f] [gen_acc: %f] [real_acc: %f] [LR_GEN: %f] [LR_ACT: %f] [Test_acc: %f] [Best Test acc: %f] [To Keep: %d]"
                    % (epoch, opt.n_epochs, i, len(dataloader), action_loss_real.item(), -1.0, -1.0, train_acc_real,optimizer_G.param_groups[0]['lr'], optimizer_hcn.param_groups[0]['lr'],\
                        last_ep_test_acc, best_test_acc, sum(toKeep))
                )
                action_loss = action_loss_real
                fakeClassifierLoss = -1.0
            ...
            gen_imgs_loss.update(fakeClassifierLoss,imgs.shape[0])
        ...
        writer.add_scalar('Total Action/Loss',generator_loss.avg,epoch)
        ...
        writer.add_scalar('Best Test acc',best_test_acc,epoch)
