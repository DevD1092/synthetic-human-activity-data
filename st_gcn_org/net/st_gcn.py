...
import numpy as np
import math
from .net import Unit2D, conv_init, import_class, Unit2D_transpose
from .unit_gcn import unit_gcn, unit_gcn_transpose

default_backbone = [(64, 64, 1), (64, 64, 1), (64, 64, 1), (64, 128,
                                                            2), (128, 128, 1),
                    (128, 128, 1), (128, 256, 2), (256, 256, 1), (256, 256, 1)]

default_backbone_transpose = [(64, 64, 1), (64, 64, 1), (64, 64, 1), (64, 128,
                                                            1), (128, 128, 1),
                    (128, 128, 1), (128, 256, 1), (256, 256, 1), (256, 3, 1)]
...
class Model(nn.Module):
    ...
    def __init__(self,
                 ...
                 graph='st_gcn_org.graph.NTU_RGB_D',
                 graph_args=dict(labeling_mode = 'spatial',num_node=25),
                 mask_learning=True,
                 use_local_bn=False,
                 multiscale=False,
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.multiscale:
            ...
            unit = TCN_GCN_unit_transpose

        # backbone
        if backbone_config is None:
            backbone_config = default_backbone_transpose
        self.backbone = nn.ModuleList([
            unit(in_c, out_c, stride=stride, **kwargs)
            for in_c, out_c, stride in backbone_config
        ])
        ...
        # head
        self.gcn0 = unit_gcn_transpose(
            32,
            backbone_in_c,
            self.A,
            mask_learning=mask_learning,
            use_local_bn=use_local_bn)
        self.tcn0 = Unit2D_transpose(backbone_in_c, backbone_in_c, kernel_size=9)
        ...
        self.linear_tailmodel = nn.Sequential(
            *block(num_point*3*window_size,1024),            
            nn.Linear(1024,num_point*3*window_size)
        )
        #self.linear_tailmodel = nn.Sequential(            
        #    nn.Linear(1024,num_point*3*window_size)
        #)
        self.img_shape = (3,window_size,num_point,num_person)

    def forward(self, z, l, C, T, V, M):
...
class TCN_GCN_unit_transpose(nn.Module):
    def __init__(self,
                 in_channel,
                 out_channel,
                 A,
                 ...
                 use_local_bn=False,
                 mask_learning=False):
        super(TCN_GCN_unit_transpose, self).__init__()
        half_out_channel = out_channel / 2
        self.A = A
        self.V = A.size()[-1]
        self.C = in_channel

        self.gcn1 = unit_gcn_transpose(
            in_channel,
            out_channel,
            A,
            use_local_bn=use_local_bn,
            mask_learning=mask_learning)
        self.tcn1 = Unit2D_transpose(
            out_channel,
            out_channel,
            kernel_size=kernel_size,
            dropout=dropout,
